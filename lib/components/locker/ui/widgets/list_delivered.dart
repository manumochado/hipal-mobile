...
class ListDelivered extends StatefulWidget{
  ...
  createState() => _ListDelivered();

}

class _ListDelivered extends State<ListDelivered>{
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<LockersBloc, LockersState>(builder: (context, state) {
      items = (state.lockersDelivered!=null ? state.lockersDelivered : [])!;
      ...
      if(items.length== 0){
        return Container(
              ...
              Text("No hay paquetes entregados para mostar",
                  style: TextStyle(
        ...
        return SmartRefresher(
            ...
            onRefresh: () async {
              BlocProvider.of<LockersBloc>(context).add( LoadRefreshLocker(context, "Entregado") );
              await Future.delayed(Duration(milliseconds: 1000));
              // if failed,use refreshFailed()
              _refreshController.refreshCompleted();
            },
            onLoading: () async {
              BlocProvider.of<LockersBloc>(context).add( LoadMoreLocker(context, "Entregado") );
              await Future.delayed(Duration(milliseconds: 1000));
